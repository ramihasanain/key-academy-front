...
const SCHEMAS = {
    ...
    teacherassistants: {
        ...
        columns: [ { key: 'name', label: 'اسم المساعد' }, { key: 'teachers_str', label: 'الأساتذة' }, { key: 'groups_str', label: 'مجموعاته' }, { key: 'students_count', label: 'عدد الطلاب', type: 'number_badge' }, { key: 'phone', label: 'رقم الهاتف' }, { key: 'total_replies', label: 'إجابات الأسئلة', type: 'number_badge' }, { key: 'total_messages', label: 'رسائل التوجيه', type: 'number_badge' }, { key: 'is_active', label: 'مفعل', type: 'boolean' } ],
    ...
    weeklyexams: { title: 'الامتحانات الأسبوعية (حسب الوحدة)', endpoint: 'weeklyexams', columns: [{ key: 'title', label: 'عنوان الامتحان' }, { key: 'module', label: 'الوحدة الأم' }, { key: 'start_time', label: 'تاريخ البدء', type: 'datetime' }, { key: 'end_time', label: 'موعد الإغلاق', type: 'datetime' }, { key: 'total_mark', label: 'العلامة' }] },
    ...
    return (
        <div className="hq-grid-wrap">
            <div className="hq-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {canAdd && (
                    <button className="hq-btn-primary" onClick={() => navigate(model === 'teacherassistants' ? '/hq/teacherassistants/new' : `/hq/${model}/new`)}>
